...
class A3CAgent(object):
    ...
    def train(
        ...
        """Worker training function for asynchronous advantage actor-critic
        (A3C).
        ...
        while True:
            ...
            for step in range(args.num_steps):
                ...
                state, reward, terminal, truncated, _ = env.step(
                    action.numpy())
                done = terminal or truncated
                # Check if episode is done
                reward = max(min(reward, 1),
                             -1)  # Clip reward to range [-1, 1]
    ...
    def test(self, rank: int, args, shared_model: torch.nn.Module,
             counter) -> None:
        """Test function for the asynchronous advantage actor-critic (A3C)
        agent. It runs a policy in a test environment and synchronizes
        periodically with a shared model to evaluate performance.
        ...
        while True:
            ...
            if done:
                ...
                ) - start_time  # Calculate elapsed time
                fps = (
                ...
                time.sleep(
                    60
                )  # Sleep for 60 seconds before starting the next episode
    ...
    def run(self, ) -> None:
        ...
        # Create a local model
        shared_model = ActorCritic(obs_dim=obs_dim,
                                   hidden_dim=args.hidden_dim,
                                   action_dim=action_dim)
