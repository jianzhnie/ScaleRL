...
from scalerl.envs.vector.pz_vec_env import PettingZooVecEnv
...
class AsyncPettingZooVecEnv(PettingZooVecEnv):
    """Vectorized PettingZoo environment that runs multiple environments in
    parallel.
    ...
    def __init__(self, env_fns, experience_spec=None, copy=True):
        ...
        dummy_env = env_fns[0]()
        self.metadata = (dummy_env.metadata if hasattr(dummy_env, 'metadata')
                         else dummy_env.unwrapped.metadata)
        self.render_mode = (dummy_env.render_mode if hasattr(
            dummy_env, 'render_mode') else dummy_env.unwrapped.render_mode)
        ...
        self._obs_buffer = SharedMemory(num_envs=self.num_envs,
    ...
    def reset(
        ...
        *,
        ...
        """Reset all the environments and return two dictionaries of batched
    ...
    def reset_wait(self, timeout=60):
        ...
        return (
            ...
            } if self.copy else self.observations),
            infos,
    ...
    def step_wait(self, timeout=60):
        ...
        rewards, terminations, truncations, infos = (defaultdict(list)
                                                     for _ in range(4))
        successes = []
        infos = {}
    ...
    def _add_info(self, vector_infos, env_info, env_num):
        ...
        for key, value in env_info.items():
            ...
            if isinstance(value, dict):
                array = self._add_info(vector_infos.get(key, {}), value,
                                       env_num)
            # Otherwise, we are a base case to group the data
                ...
                if key not in vector_infos:
                    if type(value) in [int, float, bool] or issubclass(
                        ...
                        array = np.zeros((self.num_envs, *value.shape),
                                         dtype=value.dtype)
                    elif value is None:
                        array = np.full(self.num_envs,
                                        fill_value=np.nan,
                                        dtype=np.float32)
                    else:
...
class PettingZooExperienceSpec:
    """Class for formatting experiences when being returned by a vectorized
    environment.
    ...
    def detect_space_info(self, dummy_env):
        ...
        try:
            ...
            self.action_space = {
                agent: batch_space(self.single_action_space[agent],
                                   self.num_envs)
                for agent in dummy_env.possible_agents
            }
            ...
            self.observation_space = {
                agent: batch_space(self.single_observation_space[agent],
                                   self.num_envs)
                for agent in dummy_env.possible_agents
            }
    ...
    def get_placeholder_value(self, agent, transition_name, observations=None):
        """When an agent is killed, used to obtain a placeholder value to
        return for associated experience.
    ...
    def process_transition(self, transitions, observations, transition_names):
        ...
        for transition, name in zip(transition_list, transition_names):
            transition = {
                agent: (transition[agent] if agent in transition.keys() else
                        self.get_placeholder_value(agent, name, observations))
                for agent in self.agents
            }
...
class Observations:
    ...
    def __getitem__(self, key):
        ...
        return (
            self.obs_view[:,
                          self.exp_spec.observation_boundaries[agent_idx]:self.
                          exp_spec.observation_boundaries[agent_idx + 1], ].
    ...
    def __str__(self):
        ...
        my_dic = {
            ...
            astype(dtype=self.exp_spec.single_observation_space[agent].dtype
                             ...
                             (self.exp_spec.observation_shapes[agent]))
    ...
    def get_agent_obs(self, agent, flat=True):
        ...
        obs = self.obs_view[:, self.exp_spec.observation_boundaries[agent_idx]:
                            self.exp_spec.observation_boundaries[agent_idx +
                                                                 1], ]
        if flat:
            return obs.astype(
                dtype=self.exp_spec.single_observation_space[agent].dtype)
        return obs.reshape((self.num_envs, ) + (
            self.exp_spec.observation_shapes[agent])).astype(
                dtype=self.exp_spec.single_observation_space[agent].dtype)
    ...
    def __setstate__(self, state):
        ...
        self.obs_view = np.frombuffer(
            self.shared_memory, dtype=np.float32).reshape(
                (self.num_envs, self.exp_spec.total_observation_width))


class SharedMemory:
    """Class to hold the shared memory object that each of the subprocesses
    will write their observation to.
    ...
    def __init__(self, num_envs, exp_spec, context):
        self.total_bytes = (np.dtype(np.float32).itemsize *
                            exp_spec.total_observation_width * num_envs)
    ...
    try:
        while True:
            ...
            if command == 'reset':
                ...
                autoreset = all([
                    term | trunc for term, trunc in zip(
                        terminated.values(), truncated.values())
                ])
                observations.set_env_obs(
                    index, experience_spec.dict_to_1d_array(observation))
                ...
                if name in [
                        'reset', 'step', 'close', '_setattr', '_check_spaces'
                ]:
                    raise ValueError(
