...
from scalerl.data.segment_tree import MinSegmentTree, SumSegmentTree
...
class ReplayBuffer:
    ...
    def __init__(self, memory_size, field_names, device=None):
        ...
        assert len(field_names
                   ) > 0, 'Field names must contain at least one field name.'
    ...
    def _process_transition(self, experiences, np_array=False):
        ...
        for field in self.field_names:
            ts = [
                np.expand_dims(getattr(e, field), axis=0) for e in experiences
                if e is not None
            ]
            ts = np.vstack(ts)
    ...
    def sample(self, batch_size, return_idx=False):
        ...
        """
        if return_idx:
            idxs = np.random.choice(len(self.memory),
                                    size=batch_size,
                                    replace=False)
    ...
    def save_to_memory(self, *args, is_vectorised=False):
        """Applies appropriate save_to_memory function depending on whether the
        environment is vectorised or not.
...
class MultiStepReplayBuffer(ReplayBuffer):
    """The Multi-step Experience Replay Buffer class. Used to store experiences
    and allow off-policy learning.
    ...
    def _get_n_step_info(self, n_step_buffer, gamma):
        """Returns n step reward, next_state, and done, as well as other saved
        transition elements, in order."""
...
class PrioritizedReplayBuffer(MultiStepReplayBuffer):
    """The Prioritized Experience Replay Buffer class. Used to store
    ...
    def __init__(
        ...
        super().__init__(memory_size, field_names, num_envs, n_step, gamma,
                         device)
    ...
    def sample(self, batch_size, beta=0.4):
        ...
        weights = torch.from_numpy(
            np.array([self._calculate_weight(i, beta) for i in idxs])).float()
