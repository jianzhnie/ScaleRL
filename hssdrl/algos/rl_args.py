...
class RLArguments:
    ...
    # General settings
    project: str = field(
        default="rlzero", metadata={"help": "Project name (default: rlzero)"}
    )
    algo_name: str = field(
        default="dmc", metadata={"help": "Algorithm name (default: dqn)"}
    )
    objective: str = field(
        default="adp",
        metadata={
    ...
    num_actors: int = field(
        default=4, metadata={"help": "The number of actors for each simulation device"}
    )
    num_learners: int = field(default=1, metadata={"help": "Number learner threads"})

    # Device settings
    use_cuda: bool = field(default=True, metadata={"help": "Use CUDA (default: True)"})
    ...
    # Hyperparameters
    use_lstm: bool = field(default=True, metadata={"help": "Use LSTM in agent model"})
    ...
    batch_size: int = field(default=32, metadata={"help": "Learner batch size"})
    ...
    max_grad_norm: float = field(
        default=40.0, metadata={"help": "Max norm of gradients"}
    )

    # Optimizer settings
    learning_rate: float = field(default=0.0001, metadata={"help": "Learning rate"})
    alpha: float = field(default=0.99, metadata={"help": "RMSProp smoothing constant"})
    ...
    entropy_cost: float = field(
        default=0.0006, metadata={"help": "Entropy cost/multiplier."}
    )
    baseline_cost: float = field(
        default=0.5, metadata={"help": "Baseline cost/multiplier."}
    )
    discounting: float = field(default=0.99, metadata={"help": "Discounting factor"})
    reward_clipping: str = field(
        default="abs_one", metadata={"help": "Reward clipping"}
    )

    # Model save settings
    load_model: bool = field(default=False, metadata={"help": "Load an existing model"})
    ...
    save_interval: int = field(
        default=30,
        metadata={"help": "Time interval (in minutes) at which to save the model"},
    )
